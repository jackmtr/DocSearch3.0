        ...
        public IEnumerable<PublicVM> SelectAll(string publicNumber, string role) {
            ...
            if (role == "admin")
            {
                var documentList = (from d in _db.tbl_Document //.AsNoTracking reduces resources by making this read only                        
                                    ...
                                    select new {
                                        f.Folder_ID, d.Document_ID, dt.DocumentType_ID, DtName = dt.Name, d.Issue_DT, d.Description, cat.Category_ID, CatName = cat.Name, dr.Date1_DT, dr.RefNumber, d.FileType, d.Method, d.Originator, d.Reason, dr.Number1/*, d.Recipient*/
                                    }).ToList();
